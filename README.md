...
本项目采用**专注桌面自动化的分层架构**，简洁高效：

```
┌─────────────────────────────────────────┐
│            PyQt6 GUI 层                 │  🖥️ 桌面用户界面
│  ┌─────────────────────────────────────┐│
│  │   主窗口 │ 控制面板 │ 状态显示      ││  界面组件
│  │  配置界面 │ 模板管理 │ 历史记录     ││  交互逻辑
│  └─────────────────────────────────────┘│
...
### 核心特性
- **Windows原生集成**：直接访问系统API，性能优异
- **实时图像识别**：OpenCV + AI模型，精确识别游戏状态
- **智能决策系统**：基于状态机和强化学习的自动化策略
- **轻量级架构**：专注核心功能，无复杂依赖
...
3. **安装依赖**：
   ...
   # 或最小安装（仅核心功能）
   pip install opencv-python numpy PyQt6 pyautogui psutil
   ```
