...
def ensure_directories_exist():
    ...
    for directory in directories:
        if not os.path.exists(directory):
            print(f"确保目录存在: {os.path.relpath(directory, PROJECT_ROOT)}")
...
def run_tests(test_type="all"):
    ...
    Args:
        test_type: 要运行的测试类型，可选值为 "all"、"unit"、"integration"、"functional"、"performance"、"system"
    ...
    if test_type == "all" or test_type == "integration":
        ...
        try:
            if os.path.exists(integration_tests_path):
    ...
    if test_type == "all" or test_type == "performance":
        ...
        try:
            if os.path.exists(performance_tests_path):
    ...
    if test_type == "all" or test_type == "system":
        system_tests_path = os.path.join(TESTS_DIR, "system")
        try:
            if os.path.exists(system_tests_path):
                system_tests = test_loader.discover(system_tests_path, pattern="*_test.py", top_level_dir=TESTS_DIR)
                test_suite.addTests(system_tests)
        except Exception as e:
            print(f"加载系统测试失败: {e}")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--type", choices=["all", "unit", "integration", "functional", "performance", "system"], 
