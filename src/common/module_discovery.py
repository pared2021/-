...
class ModuleDiscovery:
    ...
    def _filter_dependencies(self, dependencies: List[str]) -> List[str]:
        ...
        for dep in dependencies:
            ...
            # 只保留项目内部模块
            # 检查是否为项目内部模块（不以src.开头的相对导入）
            if any(dep.startswith(path) for path in self.config.scan_paths):
                filtered.append(dep)
            # 或者检查是否为当前项目的模块（基于模块结构）
            elif any(part in ['application', 'core', 'infrastructure', 'common', 'gui', 'services'] 
