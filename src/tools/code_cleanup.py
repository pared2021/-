    ...
    def analyze_redundant_services(self) -> Dict[str, List[str]]:
        ...
        redundant_services = {
            'game_analyzers': [
                ...
                'services/game_state_analyzer.py', 
                'core/unified_game_analyzer.py',
                'legacy/original_game_analyzer_services.py'
            ],
            'window_managers': [
                'services/window_manager.py',
                'legacy/removed/window/window_manager_zzz.py',
                'legacy/removed/window/window_manager_subdir.py'
            ],
            'error_handlers': [
                'services/error_handler.py',
                'legacy/removed/error/error_handler_core.py'
            ],
            'config_managers': [
                'services/config.py',
                'legacy/removed/config/config_manager.py',
                'legacy/removed/config/config_manager_zzz.py'
            ]
        }
        ...
        for category, files in redundant_services.items():
            ...
            for file_path in files:
                full_path = self.project_root / "src" / file_path
                if full_path.exists():
        ...
        actions = [
            {
                ...
                'files': ['services/game_analyzer.py', 'services/game_state_analyzer.py'],
                'target': 'infrastructure/services/game_analyzer_service.py'
            },
            {
            ...
            },
            {
                ...
                'description': '整合配置系统',
                'files': ['legacy/removed/config/'],
                'target': 'services/config.py'
            }
        ]
